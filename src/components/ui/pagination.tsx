...
const PaginationNext = ({
  ...
  <PaginationLink
    ...
    size="sm"
    className={cn("gap-1 pr-1 sm:pr-2.5", className)}
    {...props}
  >
...
export const TablePagination = ({
  ...
  const createHref = (page: number) => {
    // rootUrlがundefinedの場合の安全策
    const basePath = rootUrl || "/home";
    return `${basePath}?page=${page}${sort ? `&sort=${sort}` : ""}`;
  };
    ...
    <Pagination className="justify-end py-8">
      <PaginationContent>
        ...
        <PaginationItem className="rounded-sm bg-gray-200">
          <PaginationLink href={createHref(currentPage)} isActive={true}>
            {currentPage}
          </PaginationLink>
        </PaginationItem>
